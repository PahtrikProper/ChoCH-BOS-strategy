...
from .order_utils import PositionState, bybit_fee_fn, calc_liq_price_short, simulate_order_fill
...
class SellOrderEngine:
    ...
    def open_position(
        ...
        trade_value = margin_used * self.config.leverage
        qty = trade_value / entry_price
        entry_fee = bybit_fee_fn(trade_value, self.config)

        liq_price = calc_liq_price_short(entry_price, self.config.leverage)
