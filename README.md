...
- `src/` – strategy code, utilities, and orchestrators.
  - `src/choch_bos_strategy_live_trader/` – **live-trading workflow** for Bybit (optimizer + live loop).
  - `src/choch_bos_strategy_paper_trader/` – **paper-trading workflow** with simulated fills, spreads, slippage, and fees (no live orders).
  - `src/choch_bos_strategy_btc_paper_trader/` and `src/choch_bos_strategy_sol_paper_trader/` – token-specific paper-trader copies defaulted to BTCUSDT and SOLUSDT.
...
```
.
├── src/choch_bos_strategy_live_trader/        # Optimizer, live loop, Bybit client utilities
...
## Strategy package
The ChoCH/BOS trading workflow is organized under `src/choch_bos_strategy_live_trader/` with an executable module entrypoint:

- Optimize then launch live loop: `python -m choch_bos_strategy_live_trader` or `python -m choch_bos_strategy_live_trader.start`.
- Core components:
...
2. From the repo root, run the optimizer + live loop:
   ```bash
   python -m choch_bos_strategy_live_trader
   # or explicitly
   python -m choch_bos_strategy_live_trader.start
   ```
3. Artifacts:
...
5. Margin mode and leverage: `src/choch_bos_strategy_live_trader/live.py` requests **isolated 10x** (`trade_mode=1`, `leverage=10`) using `/v5/position/set-leverage`; ensure your account supports the requested mode before running.
...
```
- Token-specific variants: `python -m choch_bos_strategy_btc_paper_trader` or `python -m choch_bos_strategy_sol_paper_trader` (and their `.start` modules) default to BTCUSDT and SOLUSDT, respectively.
